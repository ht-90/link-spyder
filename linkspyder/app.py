...
def crawl_sitemap():
    if request.method == "POST":
        ...
        if url_is_valid():
            ...
            # Parse sitemap pages and extract hrefs as internal or external links
            res_int = []
            res_ext = []
            ...
            for url_a_tag in url_a_tags:
                ...
                hrefs_page = [sms.convert_to_absolute_url(url=url, href=href) for href in hrefs_page]
                hrefs_page = [sms.normalize_url(url=href) for href in hrefs_page]
                valid_hrefs = []
                for href in hrefs_page:
                    if sms._page_is_valid(url=href) and sms._page_is_in_sitemap(url=url, sitemap_locs=locs_url):
                        valid_hrefs.append(href)

                int_links = sms.extract_internal_links(hrefs=hrefs_page, domain_name=domain_name, url=loc)
                ext_links = sms.extract_external_links(hrefs=hrefs_page, domain_name=domain_name, url=loc)

                res_int.append({"source": loc, "value": 1, "target": int_links})
                res_ext.append({"source": loc, "value": 1, "target": ext_links})
            ...
            nodes_int = sms.create_nodes(sitemap_locs=locs_url, categories=category_data, url=url)
            edges_int = sms.create_edges(links=res_int, nodes=nodes_int, categories=category_data)
            category_data = sms.create_group_data(domain_name=domain_name, categories=category_data)
            nodes_int = sms.size_nodes(links=edges_int, nodes=nodes_int)
            graph_data = sms.generate_graph_data(nodes=nodes_int, edges=edges_int)
